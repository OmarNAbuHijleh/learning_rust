 ...
 * It's rusts way of modeling an evaluation that can either be successful or erroneous
 */

/*
 * Running a Subset of Tests
 * Sometimes we can have an enormous number of tests and we only want a subset.
 *
 * We can run a single test by running "cargo test <test_name_without_parenthasis>" in the terminal.You can even specify the specific module path.
 *
 * If you run "cargo test ticket" then all of the tests with "ticket" in the name will run!
 */

/*
 * Ignoring Tests
 * We can use the #[ignore] to prevent a certain test from running. Maybe a feature hasn't been built yet or it's a test that you already know is passing and to save computation. You shouldn't commit this to your source code, it's just a development tool
 */

/*
 * Showing Printed Output from Passing Tests
 * we can print information in our tests if it fails. If it passes, we have to add the command line arguments to see printed statements
 */
...
mod tests {
    ...
    #[test]
    fn museum_sells_ticket_to_increase_revenue_2() -> Result<(), String> {
        let mut museum_instance = super::Museum::new(); // NOTE: We use the super keyword here because this module is sub module. We don't have to declare Museum to be public thanks to this module being in the same file. We can also use the "crate" keyword so that we can get something from within the lib directory
        let mut museum_instance = Museum::new(); // NOTE: We can just do this thanks to our "use" at the top of the module
        museum_instance.sell_ticket();

        // Notice how we swapped this in for the "assert_eq"
        if museum_instance.revenue == 25 {
            Ok(())
        } else {
            Err(String::from(
